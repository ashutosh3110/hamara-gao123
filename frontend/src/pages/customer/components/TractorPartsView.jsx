...
import { tractorPartsCategories } from '../../../data/tractorModels';

export default function TractorPartsView({ lang, onSelectPartCategory }) {
  return (
    <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3.5 scrollbar-none">
      <div className="grid grid-cols-2 gap-3.5 pt-3">
        {tractorPartsCategories.map(part => (
          <div 
            key={part.id} 
            onClick={() => onSelectPartCategory && onSelectPartCategory(part)}
          ...
          >
            ...
            <div className="text-center w-full pt-1.5 border-t border-neutral-50">
              <h4 className="text-xs font-black text-neutral-800 leading-snug truncate">
                {part.emoji} {lang === 'hi' ? part.nameHi : part.nameEn}
              </h4>
            </div>
          </div>
        ))}
